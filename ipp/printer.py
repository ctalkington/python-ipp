...
class Printer:
    ...
    def __init__(self, uri: str):
        ...
        self.ipp = IPP(host=url.host, base_path=url.path, tls=self.secure)
    ...
    async def get_attributes(self, attributes=None):
        return await self.execute(IppOperation.GET_PRINTER_ATTRIBUTES, {
            "printer-uri": self.uri,
            "requesting-user-name": "PythonIPP",
            "requested-attributes": attributes if attributes else DEFAULT_PRINTER_ATTRIBUTES
        })

    async def get_jobs(self, which_jobs: str = "not-completed", my_jobs: bool = False, attributes=None):
        return await self.execute(IppOperation.GET_JOBS, {
            "printer-uri": self.uri,
            "which-jobs": which_jobs,
            "my-jobs": my_jobs,
            "requested-attributes": attributes + ["job-id"] if attributes else DEFAULT_JOB_ATTRIBUTES
        })
